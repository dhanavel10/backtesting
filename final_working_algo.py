...
# ───────────────────────────────────────────────────────────────────────────────
LOW_POINTS_THRESHOLD = 15   # trades capturing fewer than this many points are flagged

def build_advanced_stats(trades, df_day):
    """
    Computes:
      1. Low-point trades  : trades with Points Captured < LOW_POINTS_THRESHOLD
         + per-day count of such trades
      2. Win / Loss streaks: max consecutive wins and losses
      3. Long / Short totals
      4. Avg profit & loss per day (in points)
      5. Highest profit & highest loss per day (in points)
    ...
    # ── 1. Low-point trades ────────────────────────────────────────────────────
    low_pt = df_t[df_t["Points Captured"] < LOW_POINTS_THRESHOLD].copy()
    low_pt_total = len(low_pt)

    # Per-day count of low-point trades
    if not low_pt.empty:
    ...
    # ── 2. Win / Loss streaks ──────────────────────────────────────────────────
    results = (df_t["P&L %"] > 0).astype(int).tolist()   # 1=win, 0=loss
...
def print_results(trades, htf_enabled):
    ...
    # ── Pre-compute all aggregates up front (used across multiple sections) ───
    total    = len(df_t)
    wins     = (df_t["P&L %"] > 0).sum()
    losses   = total - wins
    ...
    # ── Low-point trades ──────────────────────────────────────────────────────
    print("\n\n" + SEP)
    print(f"  LOW-POINT TRADES  (Points Captured < {LOW_POINTS_THRESHOLD})")
    print(DASH)
    if adv["low_pt_total"] > 0:
        lp_cols = ["Date","Direction","Entry Time","Entry Price","Exit Time",
                   "Exit Price","Points Captured","P&L %","Exit Reason","Result"]
        print(adv["low_pt_trades"][lp_cols].to_string(index=False))
        print(f"\n  Total low-point trades : {adv['low_pt_total']} of {total} "
              f"({adv['low_pt_total']/total*100:.1f}%)")
        print(f"\n  Per-day breakdown of low-point trades:")
        print(adv["low_pt_daily"].to_string(index=False))
    else:
        print(f"  ✅  No trades with Points Captured < {LOW_POINTS_THRESHOLD}")
    ...
    print(f"  Total Longs         : {adv['total_longs']}  |  Won: {adv['long_wins']}  ({adv['long_wins']/adv['total_longs']*100:.1f}%)" if adv['total_longs'] > 0 else f"  Total Longs         : 0")
    print(f"  Total Shorts        : {adv['total_shorts']}  |  Won: {adv['short_wins']}  ({adv['short_wins']/adv['total_shorts']*100:.1f}%)" if adv['total_shorts'] > 0 else f"  Total Shorts        : 0")
    print(DASH)
    ...
    with pd.ExcelWriter(fname, engine="openpyxl") as writer:
        ...
        if adv:
            ...
            adv_rows = [
                ...
                ["", ""],
                ["LOW-POINT TRADES", ""],
                [f"Threshold (pts)",    LOW_POINTS_THRESHOLD],
            ...
            df_adv_summary = pd.DataFrame(adv_rows, columns=["Metric","Value"])
            df_adv_summary.to_excel(writer, sheet_name="Advanced Stats", index=False)

            # Low-point trade list
            if not adv["low_pt_trades"].empty:
                lp_cols = ["Date","Direction","Entry Time","Entry Price","Exit Time",
                           "Exit Price","Points Captured","P&L %","Exit Reason","Result"]
                adv["low_pt_trades"][lp_cols].to_excel(
                    writer, sheet_name="Low-Point Trades", index=False)

            # Low-point per-day table
            if not adv["low_pt_daily"].empty:
                adv["low_pt_daily"].to_excel(
                    writer, sheet_name="Low-Pt Per Day", index=False)
...
def build_chart(df, trades, htf_enabled):
    ...
    if htf_enabled and "HTF_bull" in df_c.columns:
        htf_bull_bool = df_c["HTF_bull"].astype(bool)   # ← fix: ensure proper bool dtype
...
def main():
    ...
    df_trades, df_day, df_ts, adv = print_results(trades, htf_enabled)

    print("  Exporting Excel …")
    export_excel(df, df_trades, df_day, df_ts, htf_enabled, adv)
