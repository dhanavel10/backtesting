...
CHANGES vs v4.3
  ...
  FIX (v4.4b) — EXIT PRICE CORRECTNESS:
    When CLOSE_BASED_EXIT=True, the exit is triggered by the candle CLOSE
    crossing the ST line. The exit price is now correctly set to c_close
    (the price you actually transacted at) instead of the ST line value
    (which price has already moved past).
    When CLOSE_BASED_EXIT=False, the exit is triggered by intracandle
    touch of the ST line, so exit_px remains c_exit_st / c_ltf_st_val
    (stop-order fill assumption — unchanged).
    Hard SL always exits at hard_sl_price regardless of this setting.
...
# ═══════════════════════════════════════════════════════════════════════════════
#  BACKTEST ENGINE  — v4.4b: EXIT PRICE FIX
# ═══════════════════════════════════════════════════════════════════════════════
            ...
            if direction == "long":
                ...
                if hard_hit or ltf_hit or st_hit:
                    if hard_hit:
                        ...
                        # ── FIX: use c_close when CLOSE_BASED_EXIT=True ──────
                        exit_px = c_close if CLOSE_BASED_EXIT else c_ltf_st_val
                        ...
                        # ── FIX: use c_close when CLOSE_BASED_EXIT=True ──────
                        exit_px = c_close if CLOSE_BASED_EXIT else c_exit_st
                    else:
                        reason  = "ST Flip Bear"
                        # ── FIX: use c_close when CLOSE_BASED_EXIT=True ──────
                        exit_px = c_close if CLOSE_BASED_EXIT else c_exit_st
...
def build_advanced_option_metrics(trades):
    ...
    SEP  = "=" * 69
    DASH = "-" * 69
    lines = ["", SEP, "  OPTION P/L ANALYTICS METRICS FRAMEWORK  (v4.4b — exit price fix)", SEP]
...
def export_html(df, trades, df_day, df_ts, htf_enabled, vix_series):
    ...
    {card("Exit Price Mode", "CLOSE" if CLOSE_BASED_EXIT else "ST LINE", "exit price basis")}
    """
    ...
    <div class="formula-box">
      <p class="formula-title">v4.4b — Exit Price Fix + 5-Point Confirmation Entry Logic</p>
      <code>EXIT PRICE RULE (NEW):</code>
      <code>  CLOSE_BASED_EXIT = True  → exit_px = c_close   (candle close — price you transacted at)</code>
      <code>  CLOSE_BASED_EXIT = False → exit_px = ST line    (stop-order fill at ST level)</code>
      <code>  Hard SL always           → exit_px = hard_sl_price  (fixed stop level)</code>
      <br>
      <code>TRIGGER CANDLE  : All existing conditions satisfied → trigger_close = candle.close</code>
      <code>LONG  CONFIRM   : next candle(s) close  &gt;  trigger_close + {ENTRY_CONFIRM_POINTS} pts  → Enter Long</code>
      <code>SHORT CONFIRM   : next candle(s) close  &lt;  trigger_close - {ENTRY_CONFIRM_POINTS} pts  → Enter Short</code>
      <code>CANCEL TRIGGER  : ST flips | HTF turns against | window closes | daily cap hit | EOD</code>
      <br>
      <code>CONFIG : CLOSE_BASED_EXIT = {CLOSE_BASED_EXIT}  |  ENTRY_CONFIRM_POINTS = {ENTRY_CONFIRM_POINTS}</code>
    </div>"""
...
<title>{SYMBOL} — Supertrend + Options Report v4.4b</title>
...
<body>
<header>
  <div class="logo">{SYMBOL} · Supertrend v4.4b · LTF ST Exit + 5-Point Confirmation + Exit Price Fix</div>
  <div class="header-top">
    <div>
      <h1>⚡ {SYMBOL} — Options Backtest Report v4.4b</h1>
      <div class="meta">
        ...
        Exit Price: {"CLOSE" if CLOSE_BASED_EXIT else "ST LINE"} &nbsp;|&nbsp;
...
<main>
  ...
  <section id="sec-overview">
    ...
    <h3>Exit Price Logic &amp; Entry Confirmation</h3>
    {confirm_info}
...
<footer>
  {SYMBOL} Supertrend v4.4b — Exit Price Fix (CLOSE_BASED_EXIT={CLOSE_BASED_EXIT}) &nbsp;·&nbsp;
  LTF ST({LTF_ST_PERIOD},{LTF_ST_MULTIPLIER}) {"ON" if LTF_ST_EXIT_ENABLED else "OFF"} &nbsp;·&nbsp;
...
{all_scripts}
    ...
    fname = f"{SYMBOL}_options_report_v4_4b.html"
...
def print_results(trades, htf_enabled):
    ...
    print(f"  {SYMBOL}  ST({ST_PERIOD},{ST_MULTIPLIER})  |  Trades: {total}  |  "
          ...
          f"Confirm: ±{ENTRY_CONFIRM_POINTS} pts  |  "
          f"Exit Price: {'CLOSE' if CLOSE_BASED_EXIT else 'ST LINE'}  |  "
    ...
    print("\n" + SEP); print("  OPTION P&L SUMMARY  (v4.4b)"); print(DASH)
...
def main():
    print("\n" + "═"*60)
    print("  SUPERTREND + OPTIONS BACKTEST v4.4b")
    print(f"  FIX: Exit price = CLOSE when CLOSE_BASED_EXIT=True")
    ...
    print(f"  CLOSE_BASED_EXIT = {CLOSE_BASED_EXIT}  →  exit_px = {'c_close' if CLOSE_BASED_EXIT else 'ST line value'}")
    ...
    print(f"\n  EXIT PRICE LOGIC (v4.4b fix):")
    print(f"  • CLOSE_BASED_EXIT=True  → exit_px = c_close  (candle close price)")
    print(f"  • CLOSE_BASED_EXIT=False → exit_px = ST line   (stop-order fill)")
    print(f"  • Hard SL always         → exit_px = hard_sl_price\n")
