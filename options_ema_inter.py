"""
Supertrend Intraday Backtesting Strategy — v4.5 + HTF Entry Gate
─────────────────────────────────────────────────────────────────────────────
CHANGES vs v4.5

  NEW — HTF SUPERTREND ENTRY GATE:
    HTF ST (15-min, ST(10,3.0)) is now used as an entry condition.
    Both the base ST (1-min) AND HTF ST must agree in direction.

    Rules:
      Long  entry : base ST bull  AND  HTF ST bull  (both agree → long)
      Short entry : base ST bear  AND  HTF ST bear  (both agree → short)

    If they disagree (e.g. HTF bull but base ST bear):
      → NO trade is opened.
      → Wait silently until base ST flips to match HTF direction,
        THEN re-evaluate all entry conditions on that flip candle.

    A pending confirmation trigger is also cancelled immediately if the
    HTF ST direction turns against the pending trade direction mid-wait.

    Config key: HTF_ENTRY_ENABLED = True   (set False to disable without
                                            removing the logic)

RETAINED from v4.5:
  EMA-9 steepness entry filter  (EMA9_SLOPE_MIN = 3.5 pts)
  Candle body filter            (CANDLE_BODY_MIN = 13.0 pts)
  5-Point Confirmation Entry    (ENTRY_CONFIRM_POINTS = 5.0)
  HTF still loaded for optional exit use (EXIT_USE_HTF_ST)
  LTF Supertrend Exit           (disabled by default)
  Greeks / Options P&L engine
"""
...
HARD_SL_PCT      = 0.25
EXIT_USE_HTF_ST  = False   # HTF ST can still be used for EXIT; separate from entry gate
CLOSE_BASED_EXIT = True

# ── LTF Supertrend Exit ────────────────────────────────────────────────────────
LTF_ST_EXIT_ENABLED  = False
LTF_ST_PERIOD        = 14
LTF_ST_MULTIPLIER    = 5.0

# ── HTF Entry Gate (NEW) ───────────────────────────────────────────────────────
# When True, both base ST and HTF ST must agree in direction before entry.
#   Long  : base ST bull  AND  HTF ST bull
#   Short : base ST bear  AND  HTF ST bear
# If they disagree → no trade; wait for base ST to flip to match HTF.
# A pending confirmation trigger is cancelled if HTF turns against it mid-wait.
# Set False to revert to v4.5 behaviour (no HTF gate on entry).
HTF_ENTRY_ENABLED = True

# ── EMA-9 Entry Filter ─────────────────────────────────────────────────────────
# Long  entry requires: close > EMA9  AND  (EMA9[i] - EMA9[i-1]) >  EMA9_SLOPE_MIN
# Short entry requires: close < EMA9  AND  (EMA9[i] - EMA9[i-1]) < -EMA9_SLOPE_MIN
EMA9_PERIOD     = 9
EMA9_SLOPE_MIN  = 4   # minimum EMA9 change (points) between consecutive candles

# ── Candle Body Filter ─────────────────────────────────────────────────────────
# Trigger candle must have |close - open| > CANDLE_BODY_MIN
CANDLE_BODY_MIN = 15.0  # points
...
def compute_ema(series, period):
    """Return a numpy array of EMA values (same length as series, NaN for warm-up)."""
    alpha = 2.0 / (period + 1)
    vals  = series.values.astype(float)
    ema   = np.full(len(vals), np.nan)
    # seed with simple average of first `period` values
    if len(vals) >= period:
        ema[period - 1] = vals[:period].mean()
        for j in range(period, len(vals)):
            ema[j] = alpha * vals[j] + (1 - alpha) * ema[j - 1]
    return ema
...
def fetch_data():
    ...
    # ── EMA-9 (computed on base 1-min close) ───────────────────────────────────
    ema9_vals = compute_ema(df["Close"], EMA9_PERIOD)
    df["EMA9"] = ema9_vals
    df.dropna(subset=["EMA9"], inplace=True)
    print(f"  ✅  EMA({EMA9_PERIOD}) computed — slope threshold: ±{EMA9_SLOPE_MIN} pts")
    ...
    # ── HTF Supertrend ─────────────────────────────────────────────────────────
    # Always loaded — used as entry gate when HTF_ENTRY_ENABLED=True
    # and optionally for exit when EXIT_USE_HTF_ST=True.
    htf_enabled = False
    if htf_source is not None:
        try:
            ...
            if htf_bull.notna().mean() >= 0.5:
                ...
                htf_enabled = True
                entry_note  = "ENTRY GATE" + (" + EXIT" if EXIT_USE_HTF_ST else "")
                print(f"  ✅  HTF ST({HTF_ST_PERIOD},{HTF_ST_MULTIPLIER}) loaded — {entry_note}")
        except FileNotFoundError:
            print("  ⚠  HTF file not found — HTF entry gate DISABLED.")

    if not htf_enabled:
        df["HTF_bull"] = True   # neutral sentinel — won't block entries
...
# ═══════════════════════════════════════════════════════════════════════════════
#  BACKTEST ENGINE  — v4.5 + HTF Entry Gate
# ═══════════════════════════════════════════════════════════════════════════════
...
def run_backtest(df, htf_enabled, vix_series):
    ...
    ema9_arr  = df["EMA9"].values.astype(float)
    times     = df.index

    ltf_st_vals  = df["LTF_ST_val"].values.astype(float)
    ltf_st_bulls = df["LTF_ST_bull"].values.astype(bool)

    # HTF entry gate is active only when data loaded AND config enabled
    htf_gate_active = htf_enabled and HTF_ENTRY_ENABLED
    ...
    # ── HTF gate helpers ──────────────────────────────────────────────────────
    def htf_allows_long(idx):
        """True if HTF gate inactive OR HTF is currently bullish."""
        if not htf_gate_active: return True
        return bool(htf_bulls[idx])

    def htf_allows_short(idx):
        """True if HTF gate inactive OR HTF is currently bearish."""
        if not htf_gate_active: return True
        return not bool(htf_bulls[idx])
    ...
    for i in range(1, len(df)):
        ...
        # EMA-9 values
        c_ema9      = ema9_arr[i]
        prev_ema9   = ema9_arr[i-1]
        ema9_slope  = c_ema9 - prev_ema9   # positive = rising, negative = falling
        ...
        if in_trade:
            if direction == "long":
                ...
                if hard_hit or ltf_hit or st_hit:
                    ...
                    # Attempt reverse-to-short — require HTF agreement
                    if (not hard_hit and not ltf_hit and not c_bull and
                            htf_allows_short(i) and
                            in_window(c_tod) and
                            daily_trades.get(date_key, 0) < MAX_TRADES_PER_DAY and
                            c_close < prev_close and
                            not np.isnan(c_ema9) and not np.isnan(prev_ema9) and
                            c_close < c_ema9 and
                            ema9_slope < -EMA9_SLOPE_MIN and
                            candle_body > CANDLE_BODY_MIN):
                ...
                if hard_hit or ltf_hit or st_hit:
                    ...
                    # Attempt reverse-to-long — require HTF agreement
                    if (not hard_hit and not ltf_hit and c_bull and
                            htf_allows_long(i) and
                            in_window(c_tod) and
                            daily_trades.get(date_key, 0) < MAX_TRADES_PER_DAY and
                            c_close > prev_close and
                            not np.isnan(c_ema9) and not np.isnan(prev_ema9) and
        ...
        if pending_direction is not None:
            # Invalidation checks — ST flip already cleared pending above.
            # NEW: also cancel if HTF turns against the pending direction mid-wait.
            st_invalid = (
                (pending_direction == "long"  and not c_bull) or
                (pending_direction == "short" and c_bull)
            )
            htf_invalid = (
                (pending_direction == "long"  and not htf_allows_long(i)) or
                (pending_direction == "short" and not htf_allows_short(i))
            )
            ...
            if st_invalid or htf_invalid or window_invalid or cap_invalid:
                clear_pending()
                # Fall through to fresh entry scan
        ...
        # ── Fresh entry scan ──────────────────────────────────────────────────
        # Standard pre-checks (unchanged from v4.5)
        ...
        # EMA-9 validity guard
        if np.isnan(c_ema9) or np.isnan(prev_ema9):
            continue

        # ── LONG (CALL BUY) entry conditions ──────────────────────────────────
        # 1. ST bullish
        # 2. HTF ST bullish (if gate active) — NEW
        # 3. Close > EMA9
        # 4. EMA9 slope > +EMA9_SLOPE_MIN
        # 5. Candle body > CANDLE_BODY_MIN
        # 6. Close > prev close
        if (c_bull and
                htf_allows_long(i) and
                c_close > c_ema9 and
                ema9_slope > EMA9_SLOPE_MIN and
                candle_body > CANDLE_BODY_MIN and
                c_close > prev_close):
            set_pending("long", c_close, date_key)

        # ── SHORT (PUT BUY) entry conditions ──────────────────────────────────
        # 1. ST bearish
        # 2. HTF ST bearish (if gate active) — NEW
        # 3. Close < EMA9
        # 4. EMA9 slope < -EMA9_SLOPE_MIN
        # 5. Candle body > CANDLE_BODY_MIN
        # 6. Close < prev close
...
# ═══════════════════════════════════════════════════════════════════════════════
#  SUMMARY BUILDERS  (unchanged from v4.5)
# ═══════════════════════════════════════════════════════════════════════════════
...
# ═══════════════════════════════════════════════════════════════════════════════
#  ADVANCED METRICS  (unchanged from v4.5)
# ═══════════════════════════════════════════════════════════════════════════════
...
def build_advanced_option_metrics(trades):
    ...
    SEP  = "=" * 69
    DASH = "-" * 69
    lines = ["", SEP, "  OPTION P/L ANALYTICS — v4.5 + HTF Entry Gate", SEP]
    ...
    def sim_pnl(ds_mult=1.0, iv_add_pts=0.0, hr_add=0.0):
        ...
        for _, row in df.iterrows():
            ...
            iv_dec   = max(-0.03, min(0.03, (vix_chg + iv_add_pts) / 100.0))
    ...
    try:
        for label, kwargs in [
            ...
            ("IV +5%",  dict(iv_add_pts=5.0)), ("IV -5%", dict(iv_add_pts=-5.0)),
...
# ═══════════════════════════════════════════════════════════════════════════════
#  PLOTLY CHART BUILDERS  (unchanged from v4.5, HTF ST line added to candle chart)
# ═══════════════════════════════════════════════════════════════════════════════
...
def build_candle_chart(df):
    ...
    fig.add_trace(go.Scatter(
        ...
        name=f"EMA({EMA9_PERIOD})", mode="lines",
        line=dict(color="#ffd700", width=1.5, dash="dot")))
    # HTF ST line — dashed blue so it's visually distinct
    if "HTF_val" in df_c.columns:
        fig.add_trace(go.Scatter(
            x=df_c.index, y=df_c["HTF_val"],
            name=f"HTF ST({HTF_ST_PERIOD},{HTF_ST_MULTIPLIER})", mode="lines",
            line=dict(color="#3d9bff", width=1.5, dash="dash")))
    fig.update_layout(
...
# ═══════════════════════════════════════════════════════════════════════════════
#  HTML REPORT  (updated entry logic box and cards; rest unchanged from v4.5)
# ═══════════════════════════════════════════════════════════════════════════════
...
def export_html(df, trades, df_day, df_ts, htf_enabled, vix_series):
    ...
    htf_gate_status = f"ON — ST({HTF_ST_PERIOD},{HTF_ST_MULTIPLIER}) 15m" if (htf_enabled and HTF_ENTRY_ENABLED) else "OFF"
    ...
    {card("HTF Entry Gate", htf_gate_status, "both ST must agree")}
    {card("EMA9 Slope Filter", f"±{EMA9_SLOPE_MIN} pts", f"EMA({EMA9_PERIOD}) steepness")}
    {card("Candle Body Filter", f">{CANDLE_BODY_MIN} pts", "|close - open|")}
    ...
    <div class="formula-box">
      <p class="formula-title">v4.5 + HTF Entry Gate — Entry Conditions</p>
      <code>LONG  (CALL buy) : base ST bull  AND  HTF ST({HTF_ST_PERIOD},{HTF_ST_MULTIPLIER}) bull  AND  close &gt; EMA({EMA9_PERIOD})  AND  EMA9[i]−EMA9[i-1] &gt; +{EMA9_SLOPE_MIN}  AND  |close−open| &gt; {CANDLE_BODY_MIN}</code>
      <code>SHORT (PUT  buy) : base ST bear  AND  HTF ST({HTF_ST_PERIOD},{HTF_ST_MULTIPLIER}) bear  AND  close &lt; EMA({EMA9_PERIOD})  AND  EMA9[i]−EMA9[i-1] &lt; −{EMA9_SLOPE_MIN}  AND  |close−open| &gt; {CANDLE_BODY_MIN}</code>
      <br>
      <code>HTF DISAGREE    : no trade — wait for base ST to flip and match HTF direction</code>
      <code>PENDING CANCEL  : pending trigger cancelled if HTF turns against it mid-wait</code>
      <br>
      <code>THEN (5-pt confirm) : wait for next candle to close {ENTRY_CONFIRM_POINTS} pts beyond trigger_close</code>
      <br>
      <code>HTF ENTRY GATE  : {"ENABLED — ST("+str(HTF_ST_PERIOD)+","+str(HTF_ST_MULTIPLIER)+") 15m" if (htf_enabled and HTF_ENTRY_ENABLED) else "DISABLED (HTF_ENTRY_ENABLED=False or HTF data not loaded)"}</code>
      <code>LTF ST EXIT     : ST({LTF_ST_PERIOD},{LTF_ST_MULTIPLIER}) {"ENABLED" if LTF_ST_EXIT_ENABLED else "DISABLED"} — early exit when price breaches tighter ST line</code>
    </div>"""
    ...
    all_scripts = (
        plot_script(candle_json, "chart-candle") + plot_script(pnl_json,   "chart-pnl")  +
        plot_script(attr_json,   "chart-attr")   + plot_script(delta_json, "chart-delta") +
        plot_script(td_json,     "chart-td")     + plot_script(be_json,    "chart-be")   +
...
<title>{SYMBOL} — Supertrend + Options Report v4.5 + HTF Gate</title>
...
<body>
<header>
  <div class="logo">{SYMBOL} · Supertrend v4.5 + HTF Entry Gate · EMA9 + Body Filter</div>
  <div class="header-top"><div>
    <h1>⚡ {SYMBOL} — Options Backtest Report v4.5 + HTF Gate</h1>
    <div class="meta">
      Base ST({ST_PERIOD},{ST_MULTIPLIER}) &nbsp;|&nbsp;
      HTF ST({HTF_ST_PERIOD},{HTF_ST_MULTIPLIER}) 15m entry gate: {"ON" if (htf_enabled and HTF_ENTRY_ENABLED) else "OFF"} &nbsp;|&nbsp;
      LTF ST: {"ON ST("+str(LTF_ST_PERIOD)+","+str(LTF_ST_MULTIPLIER)+")" if LTF_ST_EXIT_ENABLED else "OFF"} &nbsp;|&nbsp;
      EMA({EMA9_PERIOD}) slope ±{EMA9_SLOPE_MIN} &nbsp;|&nbsp; Body &gt;{CANDLE_BODY_MIN} &nbsp;|&nbsp;
...
<main>
  <div class="disclaimer">⚠ <strong>DISCLAIMER:</strong> Greek-approximation (Taylor expansion). Greeks assumed locally constant per trade. Not financial advice.</div>
  <section id="sec-overview">
    <h2>📊 Strategy Overview</h2>
    <div class="card-grid">{strat_cards}</div>
    <h3>Entry Logic</h3>{entry_logic_info}
    <h3>Option P&amp;L Summary</h3>
    <div class="card-grid">{opt_cards}</div>
  </section>
  <section id="sec-chart">
    <h2>📈 Price, EMA({EMA9_PERIOD}), Base ST &amp; HTF ST (Last {CHART_DAYS} Days)</h2>
  ...
  <section id="sec-options">
    ...
    <h3>Per-Trade Option Detail</h3>{opt_html}
  </section>
  <section id="sec-greeks">
    <h2>⚗️ Greeks Attribution</h2>
    <div class="chart-row">{plot_div(attr_json,"chart-attr")}{plot_div(delta_json,"chart-delta")}</div>
    <div class="chart-row">{plot_div(td_json,"chart-td")}{plot_div(be_json,"chart-be")}</div>
  </section>
  <section id="sec-eqdd">
    <h2>📉 Equity Curve &amp; Drawdown</h2>{plot_div(eq_dd_json, "chart-eqdd")}
  </section>
  {vix_section}
  ...
  <section id="sec-daily">
    <h2>📅 Daily P&amp;L Breakdown</h2>{day_html}
  </section>
  <section id="sec-timeslot">
    <h2>🕐 Time Slot P&amp;L Analysis</h2>{ts_html}
  </section>
  <section id="sec-model">
    <h2>📐 Model Parameters &amp; Entry Logic</h2>{entry_logic_info}
  </section>
</main>
<footer>
  {SYMBOL} Supertrend v4.5 + HTF Gate {"ON" if (htf_enabled and HTF_ENTRY_ENABLED) else "OFF"} |
  EMA({EMA9_PERIOD}) slope±{EMA9_SLOPE_MIN} | Body&gt;{CANDLE_BODY_MIN} |
  LTF ST {"ON" if LTF_ST_EXIT_ENABLED else "OFF"} | ±{ENTRY_CONFIRM_POINTS}pt confirm |
  Δ={OPTION_DELTA} Γ={OPTION_GAMMA} Θ={OPTION_THETA}/d Lot={OPTION_LOT_SIZE} | Slip:₹{SLIPPAGE_PER_CONTRACT}
</footer>
<script>{all_scripts}</script>
</body>
</html>"""

    fname = f"{SYMBOL}_options_report_v4_5_htf.html"
...
def print_results(trades, htf_enabled):
    ...
    total = len(df_t); wins = (df_t["P&L %"] > 0).sum()
    htf_gate_status = f"ON ST({HTF_ST_PERIOD},{HTF_ST_MULTIPLIER})" if (htf_enabled and HTF_ENTRY_ENABLED) else "OFF"
    print("\n" + SEP)
    print(f"  {SYMBOL}  ST({ST_PERIOD},{ST_MULTIPLIER})  |  Trades: {total}  |  Win Rate: {wins/total*100:.1f}%  |  "
          f"HTF entry gate: {htf_gate_status}  |  "
          f"EMA{EMA9_PERIOD} slope ±{EMA9_SLOPE_MIN} | Body >{CANDLE_BODY_MIN} | "
          f"Confirm ±{ENTRY_CONFIRM_POINTS} pts  |  Slip: ₹{SLIPPAGE_PER_CONTRACT}/trade")
    print(SEP)
    ...
    print("\n" + SEP); print("  OPTION P&L SUMMARY  (v4.5 + HTF Gate)"); print(DASH)
...
def main():
    print("\n" + "═"*65)
    print("  SUPERTREND + OPTIONS BACKTEST v4.5 + HTF Entry Gate")
    print(f"  HTF entry gate    : ST({HTF_ST_PERIOD},{HTF_ST_MULTIPLIER}) 15m  {'ENABLED' if HTF_ENTRY_ENABLED else 'DISABLED'}")
    print(f"  Agreement rule    : base ST + HTF ST must BOTH agree in direction")
    print(f"  If disagree       : wait for base ST to flip and match HTF")
    print(f"  EMA({EMA9_PERIOD}) slope      : ±{EMA9_SLOPE_MIN} pts  (EMA9[i] - EMA9[i-1])")
    print(f"  Candle body       : |close - open| > {CANDLE_BODY_MIN} pts")
    print(f"  LTF ST Exit       : ST({LTF_ST_PERIOD},{LTF_ST_MULTIPLIER})  {'ENABLED' if LTF_ST_EXIT_ENABLED else 'DISABLED'}")
    print(f"  5-pt confirm      : ±{ENTRY_CONFIRM_POINTS} pts")
    ...
    print(f"\n  ENTRY LOGIC SUMMARY (v4.5 + HTF Gate):")
    print(f"  LONG  : base ST bull  AND  HTF ST bull  AND  close > EMA{EMA9_PERIOD}  AND  slope > +{EMA9_SLOPE_MIN}  AND  |body| > {CANDLE_BODY_MIN}")
    print(f"  SHORT : base ST bear  AND  HTF ST bear  AND  close < EMA{EMA9_PERIOD}  AND  slope < -{EMA9_SLOPE_MIN}  AND  |body| > {CANDLE_BODY_MIN}")
    print(f"  WAIT  : HTF disagrees → no trade until base ST flips to match HTF")
    print(f"  THEN  : wait for ±{ENTRY_CONFIRM_POINTS}pt confirmation close\n")
