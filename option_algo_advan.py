"""
Supertrend Intraday Backtesting Strategy — v4.5 (Greeks / Options Edition)
─────────────────────────────────────────────────────────────────────────────
CHANGES vs v4.4
  NEW — 4 ADDITIONAL EXIT CONDITIONS (all toggle True/False independently):

  A. ATR TRAILING STOP
     trail_stop = highest_high (long) or lowest_low (short) - ATR × multiplier
     Exit when close < trail_stop (long) or close > trail_stop (short).
     Profit measured in underlying points. Protects captured gains.

  B. BREAK-EVEN STOP
     Once the trade has gained >= BE_TRIGGER_POINTS in the underlying,
     the stop-loss is moved to entry price. Any subsequent ST/LTF/ATR stop
     cannot exit below entry — preserving capital.

  C. TIME-BASED EXIT
     If trade has been open > TIME_EXIT_MINUTES minutes AND profit in
     underlying < TIME_EXIT_MIN_PROFIT_PTS points, exit at close.
     Avoids holding stale, low-momentum trades.

  D. MOMENTUM LOSS EXIT
     If 3 consecutive candles each close lower (long) or higher (short)
     than the previous AND the cumulative move across those 3 candles
     has wiped >= 50% of the best profit captured, exit immediately.
     Profit = underlying points captured from entry to current candle.

  NOTE: All profit/point comparisons use the UNDERLYING price, NOT option P&L.

  PRIORITY ORDER (first triggered wins):
    1. Hard SL
    2. Break-even SL (moves hard_sl to entry once triggered)
    3. ATR Trailing Stop
    4. LTF ST Exit
    5. Momentum Loss Exit
    6. Time-Based Exit
    7. Main ST Exit / ST Flip
    8. EOD Exit
...
LTF_ST_MULTIPLIER    = 4.5            # tighter multiplier → closer stop line

# ── A. ATR Trailing Stop ───────────────────────────────────────────────────────
# Computes a dynamic trailing stop each candle:
#   Long  : trail_stop = highest_high_since_entry - ATR(candle) × ATR_TRAIL_MULT
#   Short : trail_stop = lowest_low_since_entry   + ATR(candle) × ATR_TRAIL_MULT
# Exit when close breaks trail_stop.
# All comparisons are in UNDERLYING POINTS (not option P&L).
ATR_TRAIL_ENABLED    = False           # ← set False to disable
ATR_TRAIL_PERIOD     = 14             # ATR lookback period (uses pre-computed ST ATR)
ATR_TRAIL_MULT       = 2.5            # trail distance = ATR × this multiplier

# ── B. Break-Even Stop ────────────────────────────────────────────────────────
# Once profit in the underlying reaches BE_TRIGGER_POINTS, the hard stop is
# moved to entry price.  From that point, any exit (ST, ATR, etc.) cannot
# trigger below/above entry — the trade is free-rolled with zero loss risk.
# Profit = |close - entry_price| in underlying points.
BE_STOP_ENABLED      = False           # ← set False to disable
BE_TRIGGER_POINTS    = 100           # underlying points profit needed to activate BE

# ── C. Time-Based Exit ────────────────────────────────────────────────────────
# If a trade has been open longer than TIME_EXIT_MINUTES AND the underlying
# profit is still below TIME_EXIT_MIN_PROFIT_PTS, exit at the candle close.
# Avoids sitting in stalled, low-momentum trades.
# Profit = underlying points from entry to current close.
TIME_EXIT_ENABLED        = False       # ← set False to disable
TIME_EXIT_MINUTES        = 30         # minimum holding time before check activates
TIME_EXIT_MIN_PROFIT_PTS = 20.0       # exit if profit (underlying pts) < this after timeout

# ── D. Momentum Loss Exit ─────────────────────────────────────────────────────
# If MOMENTUM_CANDLES consecutive candles each move against the trade direction
# AND together they have wiped >= MOMENTUM_WIPEOUT_PCT of the best profit ever
# captured in the underlying during this trade — exit at the close of the last
# consecutive candle.
# Profit = underlying points (not option P&L).
MOMENTUM_EXIT_ENABLED    = False       # ← set False to disable
MOMENTUM_CANDLES         = 3          # how many consecutive adverse candles
MOMENTUM_WIPEOUT_PCT     = 0.50       # 0.50 = 50% of peak profit wiped → exit
...
def fetch_data():
    ...
    # ── ATR column for trailing stop ─────────────────────────────────────────
    # Compute ATR(ATR_TRAIL_PERIOD) and attach to df so the backtest loop can
    # read the current ATR value at each candle without recalculating.
    # Uses the same Wilder smoothing as compute_supertrend().
    if ATR_TRAIL_ENABLED:
        ...
        _tr = np.maximum(_h[1:] - _l[1:],
              np.maximum(np.abs(_h[1:] - _c[:-1]),
                         np.abs(_l[1:] - _c[:-1])))
        _atr = np.full(_n, np.nan)
        _p   = ATR_TRAIL_PERIOD
        if _n > _p:
            _atr[_p] = _tr[:_p].mean()
            for _j in range(_p + 1, _n):
                _atr[_j] = (_atr[_j-1] * (_p - 1) + _tr[_j-1]) / _p
        df["ATR_trail"] = _atr
        print(f"  ✅  ATR TRAIL ENABLED — ATR({ATR_TRAIL_PERIOD}) × {ATR_TRAIL_MULT}")
    else:
        df["ATR_trail"] = np.nan
        print(f"  ⚪  ATR TRAIL EXIT DISABLED")
...
# ═══════════════════════════════════════════════════════════════════════════════
#  BACKTEST ENGINE  — v4.5: 5-POINT CONFIRM + 4 NEW EXIT CONDITIONS
# ═══════════════════════════════════════════════════════════════════════════════
...
def run_backtest(df, htf_enabled, vix_series):
    ...
    # ── NEW exit-condition state (reset in open_trade each time) ─────────────
    atr_trail_stop    = None   # A: current ATR trailing stop level
    be_activated      = False  # B: True once break-even stop has been triggered
    consec_adv_count  = 0      # D: consecutive adverse candle counter
    peak_profit_pts   = 0.0    # D: best underlying profit seen this trade (for wipeout %)
    ...
    # ATR array for trailing stop — nan when ATR_TRAIL_ENABLED=False
    atr_vals = df["ATR_trail"].values.astype(float)
    ...
    def open_trade(dir_, price, time_, high_, low_, date_key):
        ...
        nonlocal atr_trail_stop, be_activated, consec_adv_count, peak_profit_pts
        in_trade      = True
        direction     = dir_
        entry_price   = price
        entry_time    = time_
        ...
        # Reset per-trade exit state
        atr_trail_stop   = None    # will be set on first candle with valid ATR
        be_activated     = False
        consec_adv_count = 0
        peak_profit_pts  = 0.0
    ...
    for i in range(1, len(df)):
        ...
        if in_trade:
            if direction == "long":
                ...
                # ── Current underlying profit in points ───────────────────────
                current_profit_pts = c_close - entry_price   # positive = profit

                # ── A. ATR Trailing Stop — update each candle ─────────────────
                c_atr = atr_vals[i]
                if ATR_TRAIL_ENABLED and not np.isnan(c_atr):
                    new_trail = peak - c_atr * ATR_TRAIL_MULT
                    # Trail stop only moves UP (never relaxes)
                    if atr_trail_stop is None:
                        atr_trail_stop = new_trail
                    else:
                        atr_trail_stop = max(atr_trail_stop, new_trail)

                # ── B. Break-Even Stop — activate once profit >= threshold ────
                if BE_STOP_ENABLED and not be_activated:
                    if current_profit_pts >= BE_TRIGGER_POINTS:
                        be_activated  = True
                        # Move hard stop up to entry price (floor at entry)
                        hard_sl_price = entry_price
                        # If no hard SL was set, create one at entry now
                        if hard_sl_price is None:
                            hard_sl_price = entry_price

                # ── D. Momentum tracking — update peak profit & adverse count ─
                if current_profit_pts > peak_profit_pts:
                    peak_profit_pts  = current_profit_pts
                    consec_adv_count = 0   # new high → reset adverse streak
                else:
                    # candle closed lower than previous — adverse move
                ...
                hard_hit = HARD_SL_ENABLED and hard_sl_price is not None and c_low <= hard_sl_price

                atr_hit  = (ATR_TRAIL_ENABLED
                            and atr_trail_stop is not None
                            and c_close < atr_trail_stop)
                ...
                # ── C. Time-Based Exit ────────────────────────────────────────
                hold_mins_so_far = int((c_time - entry_time).total_seconds() // 60)
                time_hit = (TIME_EXIT_ENABLED
                            and hold_mins_so_far >= TIME_EXIT_MINUTES
                            and current_profit_pts < TIME_EXIT_MIN_PROFIT_PTS)

                # ── D. Momentum Loss Exit ─────────────────────────────────────
                profit_wiped = (peak_profit_pts - current_profit_pts)
                wipeout_pct  = (profit_wiped / peak_profit_pts) if peak_profit_pts > 0 else 0.0
                momentum_hit = (MOMENTUM_EXIT_ENABLED
                                and consec_adv_count >= MOMENTUM_CANDLES
                                and wipeout_pct >= MOMENTUM_WIPEOUT_PCT)

                if hard_hit or atr_hit or ltf_hit or momentum_hit or time_hit or st_hit:
                    # Priority: Hard SL > ATR trail > LTF ST > Momentum > Time > Main ST
                    if hard_hit:
                        reason  = f"Hard SL ({HARD_SL_PCT}%)" + (" [BE]" if be_activated else "")
                        exit_px = hard_sl_price
                    elif atr_hit:
                        reason  = f"ATR Trail ({ATR_TRAIL_MULT}x)"
                        exit_px = atr_trail_stop
                    elif ltf_hit:
                        reason  = f"LTF ST Exit ({LTF_ST_MULTIPLIER}x)"
                        exit_px = c_ltf_st_val
                    elif momentum_hit:
                        reason  = f"Momentum Loss ({MOMENTUM_CANDLES}c {int(MOMENTUM_WIPEOUT_PCT*100)}%)"
                        exit_px = c_close
                    elif time_hit:
                        reason  = f"Time Exit ({TIME_EXIT_MINUTES}m <{TIME_EXIT_MIN_PROFIT_PTS}pts)"
                        exit_px = c_close
                    ...
                    # Attempt immediate reverse-to-short (only on clean ST exits)
                    clean_st_exit = st_hit and not hard_hit and not ltf_hit and not atr_hit
                    if (clean_st_exit and not c_bull and
                ...
                # ── Current underlying profit in points ───────────────────────
                current_profit_pts = entry_price - c_close   # positive = profit

                # ── A. ATR Trailing Stop ──────────────────────────────────────
                c_atr = atr_vals[i]
                if ATR_TRAIL_ENABLED and not np.isnan(c_atr):
                    new_trail = peak + c_atr * ATR_TRAIL_MULT
                    # Trail stop only moves DOWN (never relaxes for short)
                    if atr_trail_stop is None:
                        atr_trail_stop = new_trail
                    else:
                        atr_trail_stop = min(atr_trail_stop, new_trail)
                ...
                if current_profit_pts > peak_profit_pts:
                    ...
                    if c_close > prev_close:   # adverse for short = price rising
                        consec_adv_count += 1
                    else:
                        consec_adv_count = 0

                # ── Evaluate all exit conditions ──────────────────────────────
                hard_hit = HARD_SL_ENABLED and hard_sl_price is not None and c_high >= hard_sl_price
                ...
                if hard_hit or atr_hit or ltf_hit or momentum_hit or time_hit or st_hit:
                    ...
                    # Attempt immediate reverse-to-long (only on clean ST exits)
                    clean_st_exit = st_hit and not hard_hit and not ltf_hit and not atr_hit
                    if (clean_st_exit and c_bull and
...
<title>{SYMBOL} — Supertrend + Options Report v4.5</title>
...
<body>
<header>
  <div class="logo">{SYMBOL} · Supertrend v4.5 · ATR Trail · BE Stop · Time Exit · Momentum Exit</div>
  <div class="header-top">
    <div>
      <h1>⚡ {SYMBOL} — Options Backtest Report v4.5</h1>
      <div class="meta">
        ST({ST_PERIOD},{ST_MULTIPLIER}) &nbsp;|&nbsp; HTF: {"ON" if htf_enabled else "OFF"} &nbsp;|&nbsp;
        LTF ST: {"ON ("+str(LTF_ST_MULTIPLIER)+"x)" if LTF_ST_EXIT_ENABLED else "OFF"} &nbsp;|&nbsp;
        ATR Trail: {"ON ("+str(ATR_TRAIL_MULT)+"x)" if ATR_TRAIL_ENABLED else "OFF"} &nbsp;|&nbsp;
        BE Stop: {"ON ("+str(BE_TRIGGER_POINTS)+"pts)" if BE_STOP_ENABLED else "OFF"} &nbsp;|&nbsp;
        Time Exit: {"ON ("+str(TIME_EXIT_MINUTES)+"m)" if TIME_EXIT_ENABLED else "OFF"} &nbsp;|&nbsp;
        Momentum: {"ON ("+str(MOMENTUM_CANDLES)+"c/"+str(int(MOMENTUM_WIPEOUT_PCT*100))+"%)" if MOMENTUM_EXIT_ENABLED else "OFF"} &nbsp;|&nbsp;
...
<footer>
  {SYMBOL} Supertrend v4.5 — LTF ST Exit ST({LTF_ST_PERIOD},{LTF_ST_MULTIPLIER}) {"ON" if LTF_ST_EXIT_ENABLED else "OFF"} &nbsp;·&nbsp;
...
{all_scripts}
    ...
    fname = f"{SYMBOL}_options_report_v4_5.html"
...
def print_results(trades, htf_enabled):
    ...
    print("\n" + SEP); print("  OPTION P&L SUMMARY  (v4.5)"); print(DASH)
...
def main():
    print("\n" + "═"*60)
    print("  SUPERTREND + OPTIONS BACKTEST v4.5")
    print(f"  ATR Trail  : {'ON  mult='+str(ATR_TRAIL_MULT) if ATR_TRAIL_ENABLED else 'OFF'}")
    print(f"  BE Stop    : {'ON  trigger='+str(BE_TRIGGER_POINTS)+'pts' if BE_STOP_ENABLED else 'OFF'}")
    print(f"  Time Exit  : {'ON  '+str(TIME_EXIT_MINUTES)+'m / <'+str(TIME_EXIT_MIN_PROFIT_PTS)+'pts' if TIME_EXIT_ENABLED else 'OFF'}")
    print(f"  Momentum   : {'ON  '+str(MOMENTUM_CANDLES)+'c / '+str(int(MOMENTUM_WIPEOUT_PCT*100))+'% wipeout' if MOMENTUM_EXIT_ENABLED else 'OFF'}")
    print(f"  LTF ST     : {'ON  ST('+str(LTF_ST_PERIOD)+','+str(LTF_ST_MULTIPLIER)+')' if LTF_ST_EXIT_ENABLED else 'OFF'}")
    print(f"  5pt Confirm: ±{ENTRY_CONFIRM_POINTS} pts")
