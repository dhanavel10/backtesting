"""
╔══════════════════════════════════════════════════════════════════╗
║   SPEED DEMON SCALPER  v3.1  — "SURGICAL ENTRY + BREAKOUT"     ║
╠══════════════════════════════════════════════════════════════════╣
║  WHAT'S NEW vs v3                                                ║
║  ─────────────────────────────────────────────────────────────  ║
║  ✦ EMA Acceleration Breakout Mode  [NEW v3.1]                   ║
║      → Captures strong trending moves WITHOUT waiting for        ║
║        pullback to EMA9 — fires on the breakout candle itself   ║
║      → EMA9 > EMA21 > EMA50 perfectly stacked (longs)           ║
║      → EMA gap (EMA9 - EMA21) widening vs N bars ago            ║
║      → ADX rising AND above BREAKOUT_ADX_MIN (default 25)       ║
║      → Mutually exclusive with pullback entries (no overlap)     ║
║      → Toggle: ENABLE_BREAKOUT = True / False                   ║
║      → Shared MAX_TRADES_PER_DAY counter with pullback trades   ║
║      → Same SL multipliers as existing (1.2x long / 0.75x short)║
║      → entry_type column in CSV: 'pullback' vs 'breakout'       ║
║                                                                  ║
║  ALL v3 features preserved unchanged:                            ║
║  ✦ ADX Regime Filter (ADX >= ADX_MIN for pullback entries)      ║
║  ✦ Soft EOD (trail stays alive until 15:25)                    ║
║  ✦ Tighter Short Entry: 3-bar confirmation                      ║
║  ✦ Session Gate: Midday & Euro toggleable                       ║
║  ✦ Lower BE threshold for longs (0.2%)                          ║
║  ✦ Consecutive-loss daily circuit breaker                       ║
║  ✦ Full per-trade MAE/MFE + Options P&L in CSV                  ║
╠══════════════════════════════════════════════════════════════════╣
...
║  PHASE 4 — Asymmetric Entry Logic (Pullback + Breakout)         ║
...
Usage:
    python speed_demon_v3_breakout.py              ← yfinance (default)
    python speed_demon_v3_breakout.py 5m.csv       ← CSV file
"""
...
# ── ADX Regime Filter ───────────────────────────────────────────
ADX_PERIOD       = 14
ADX_MIN          = 18          # skip pullback trade if ADX < this

# ── Asymmetric SL (pullback entries) ────────────────────────────
...
# ── Max trades per day (shared: pullback + breakout) ─────────────
...
EURO_START          = time(14, 30)
EURO_END            = time(15, 0)
SQUAREOFF_START     = time(15, 0)

# ── Soft EOD settings ───────────────────────────────────────────
EOD_HARD_EXIT       = time(15, 25)
EOD_SOFT_EXIT       = time(15, 10)


# ══════════════════════════════════════════════════════════════════
#   EMA ACCELERATION BREAKOUT CONFIG  [NEW v3.1]
#   ─────────────────────────────────────────────────────────────
#   Toggle ENABLE_BREAKOUT to switch the entire breakout layer
#   on/off. Nothing else in the strategy is affected.
# ══════════════════════════════════════════════════════════════════

ENABLE_BREAKOUT         = True   # ← MASTER TOGGLE: True = ON, False = OFF

BREAKOUT_ADX_MIN        = 25     # stricter ADX floor vs pullback (18)
                                  # ensures only strong-trend bars qualify

BREAKOUT_GAP_LOOKBACK   = 2      # compare current EMA gap vs N bars ago
                                  # smaller = more sensitive, larger = more lag

BREAKOUT_ADX_LOOKBACK   = 3      # ADX must be higher than N bars ago
                                  # confirms momentum is building not fading

BREAKOUT_LONG_SL_MULT   = 1.2    # same as existing LONG_ATR_SL_MULT
BREAKOUT_SHORT_SL_MULT  = 0.75   # same as existing SHORT_ATR_SL_MULT
BREAKOUT_BE_PCT_LONG    = 0.2    # same as existing LONG_BE_PCT
BREAKOUT_BE_PCT_SHORT   = 0.3    # same as existing SHORT_BE_PCT


# ══════════════════════════════════════════════════════════════════
#   OPTIONS P&L CONFIGURATION
# ══════════════════════════════════════════════════════════════════
...
def compute_indicators_5m(df: pd.DataFrame) -> pd.DataFrame:
    ...
    # ── Core indicators (unchanged from v3) ─────────────────────
    ...
    # Consecutive bearish bars (for short confirmation)
    ...
    # ── EMA Acceleration Breakout indicators [NEW v3.1] ─────────
    # Long breakout: EMA9 - EMA21 gap widening signals acceleration
    df['ema_gap_long']         = df['ema_fast'] - df['ema_slow']
    df['ema_gap_long_prev']    = df['ema_gap_long'].shift(BREAKOUT_GAP_LOOKBACK)
    df['ema_gap_accel_long']   = df['ema_gap_long'] > df['ema_gap_long_prev']

    # Short breakout: EMA21 - EMA9 gap widening (bear side)
    df['ema_gap_short']        = df['ema_slow'] - df['ema_fast']
    df['ema_gap_short_prev']   = df['ema_gap_short'].shift(BREAKOUT_GAP_LOOKBACK)
    df['ema_gap_accel_short']  = df['ema_gap_short'] > df['ema_gap_short_prev']

    # ADX rising: current ADX > N bars ago
    df['adx_prev']             = df['adx'].shift(BREAKOUT_ADX_LOOKBACK)
...
def chop_filters_pass(close: float, ema_fast: float, ema_slow: float,
    ...
    """Anti-chop filters for PULLBACK entries (unchanged from v3)."""
    if adx_val < ADX_MIN:
        return False
...
def run_backtest(df: pd.DataFrame) -> tuple:
    ...
    # ── Breakout indicator arrays [NEW v3.1] ─────────────────────
    ema_gap_accel_long  = df['ema_gap_accel_long'].values
    ema_gap_accel_short = df['ema_gap_accel_short'].values
    ...
    entry_price         = 0.0
    entry_time          = None
    entry_type          = 'pullback'   # NEW: 'pullback' or 'breakout'
    stop_loss           = 0.0
    be_triggered        = False
    be_level            = 0.0
    trail_active        = False
    ...
    def do_enter(dir_str, close_price, ts_now, atr_val, etype='pullback'):
        nonlocal in_trade, direction, entry_price, entry_time, entry_type
        nonlocal stop_loss, be_triggered, be_level, trail_active
        nonlocal sl_dist_initial, trade_max_favor, trade_max_adverse

        # Choose SL multiplier based on entry type
        if etype == 'breakout':
            sl_mult = BREAKOUT_LONG_SL_MULT if dir_str == 'long' else BREAKOUT_SHORT_SL_MULT
        else:
        ...
        entry_time        = ts_now
        entry_type        = etype         # tag trade type for CSV
        be_triggered      = False
        trail_active      = False
        ...
        # BE levels same for both entry types
        if direction == 'long':
    ...
    def do_exit(exit_price, ts_now, reason):
        ...
        trades.append({
            # ── Core trade fields (v3 unchanged) ──────────────
            'entry_type':        entry_type,       # NEW v3.1
            'direction':         direction,
    ...
    for idx in range(n):
        ...
        # Breakout bar values [NEW v3.1]
        gap_accel_l  = bool(ema_gap_accel_long[idx])  if not pd.isna(ema_gap_accel_long[idx])  else False
        gap_accel_s  = bool(ema_gap_accel_short[idx]) if not pd.isna(ema_gap_accel_short[idx]) else False
        adx_prev_v   = float(adx_prev_vals[idx])      if not np.isnan(adx_prev_vals[idx])      else 0.0
        ...
        # ══════════════════════════════════════════════════════════
        #   MANAGE OPEN TRADE — Dynamic Exit Engine (unchanged v3)
        # ══════════════════════════════════════════════════════════
        if in_trade:
            if direction == 'long':
        ...
        if not in_trade:
            ...
            # ── PULLBACK LONG (original v3 — unchanged) ───────────
            if ENABLE_LONG and htf_long:
                if chop_filters_pass(close, ef, es, slope, adx_v, session):
                    if (ef > es                         and
                        close > em                      and
                        slope > 0                       and
                        abs(close - ef) <= retest_tol   and
                        close > es):
                        do_enter('long', close, ts, atr, etype='pullback')
                        daily_trades[date_str] = daily_trades.get(date_str, 0) + 1

            # ── PULLBACK SHORT (original v3 — unchanged) ──────────
            if not in_trade and ENABLE_SHORT:
                if chop_filters_pass(close, ef, es, slope, adx_v, session):
                    if (ef < es                         and
                        ...
                        do_enter('short', close, ts, atr, etype='pullback')
                        daily_trades[date_str] = daily_trades.get(date_str, 0) + 1

            # ══════════════════════════════════════════════════════
            #   EMA ACCELERATION BREAKOUT ENTRIES [NEW v3.1]
            #   ─────────────────────────────────────────────────
            #   Only fires when ENABLE_BREAKOUT = True
            #   Mutually exclusive with pullback via retest_tol check
            #   Shares MAX_TRADES_PER_DAY counter
            # ══════════════════════════════════════════════════════

            # ── BREAKOUT LONG ──────────────────────────────────────
            if (ENABLE_BREAKOUT and not in_trade and ENABLE_LONG and htf_long):
                long_breakout = (
                    ef > es                          and   # EMA9 > EMA21
                    es > em                          and   # EMA21 > EMA50 (perfect stack)
                    close > em                       and   # price above macro
                    gap_accel_l                      and   # EMA gap widening
                    adx_v  > BREAKOUT_ADX_MIN        and   # strong trend gate
                    adx_v  > adx_prev_v              and   # ADX rising (momentum building)
                    slope  > 0                       and   # slope positive
                    abs(close - ef) > retest_tol           # NOT a pullback bar (no overlap)
                )
                if long_breakout:
                    do_enter('long', close, ts, atr, etype='breakout')
                    daily_trades[date_str] = daily_trades.get(date_str, 0) + 1

            # ── BREAKOUT SHORT ─────────────────────────────────────
            if (ENABLE_BREAKOUT and not in_trade and ENABLE_SHORT):
                short_breakout = (
                    ef < es                          and   # EMA9 < EMA21
                    es < em                          and   # EMA21 < EMA50 (perfect bear stack)
                    close < em                       and   # price below macro
                    gap_accel_s                      and   # bear gap widening
                    adx_v  > BREAKOUT_ADX_MIN        and   # strong trend gate
                    adx_v  > adx_prev_v              and   # ADX rising
                    slope  < 0                       and   # slope negative
                    abs(close - ef) > retest_tol     and   # NOT a pullback bar (no overlap)
                    cb >= SHORT_CONFIRM_BARS                # same short confirmation
                )
                if short_breakout:
                    do_enter('short', close, ts, atr, etype='breakout')
                    daily_trades[date_str] = daily_trades.get(date_str, 0) + 1
...
def print_results(tdf: pd.DataFrame):
    ...
    print(SEP)
    print(f"{'#':<5} {'Type':<10} {'Dir':<6} {'Entry Time':<22} {'Exit Time':<22}"
          f" {'Entry':>9} {'Exit':>9} {'SL@Entry':>10} {'SL@Exit':>9}"
          f" {'MFE':>7} {'MAE':>7} {'P&L':>9}  Reason")
    print(SEP)
    for i, r in tdf.iterrows():
        tag = r.get('entry_type', 'pullback').upper()
        print(f"{i+1:<5} {tag:<10} {r['direction'].upper():<6} {fmt_ts(r['entry_time']):<22}"
    ...
    # ── Entry Type Breakdown [NEW v3.1] ──────────────────────────
    print(f"\n{'─'*62}")
    print("  ENTRY TYPE BREAKDOWN  (Pullback vs Breakout)")
    print(f"{'─'*62}")
    for etype in ['pullback', 'breakout']:
        sub = tdf[tdf['entry_type'] == etype] if 'entry_type' in tdf.columns else pd.DataFrame()
        if sub.empty:
            print(f"  {etype.upper():<10}  no trades")
            continue
        pnl_s = sub['pnl']
        w = (pnl_s > 0).sum()
        print(f"  {etype.upper():<10}  trades={len(sub)}  wins={w}  "
              f"wr={w/len(sub)*100:.1f}%  total={pnl_s.sum():.1f}  "
              f"avg={pnl_s.mean():.1f}  "
              f"avg_mfe={sub['mfe_pts'].mean():.1f}  "
              f"avg_mae={sub['mae_pts'].mean():.1f}  "
              f"pf={round(pnl_s[pnl_s>0].sum()/abs(pnl_s[pnl_s<=0].sum()),2) if pnl_s[pnl_s<=0].sum()!=0 else 'inf'}")
    ...
    if not sl_hits.empty:
        print(f"  SL trades where MAE < median MAE: "
...
# ══════════════════════════════════════════════════════════════════
#   OPTIONS REPORT  (unchanged from v3)
# ══════════════════════════════════════════════════════════════════
...
def print_option_report(tdf: pd.DataFrame):
    ...
    print(f"\n{SEP2}")
    print("  OPTION P/L ANALYTICS METRICS FRAMEWORK  (v3.1 — Speed Demon Scalper)")
    print(SEP2)
...
def main():
    BAR = '═' * 68
    print(f"\n{BAR}")
    print("  SPEED DEMON SCALPER  v3.1  — SURGICAL ENTRY + BREAKOUT")
    print(f"{BAR}")
    ...
    if mode == 'yfinance':
        ...
        save_base = os.path.join(os.getcwd(),
                                 YFINANCE_SYMBOL.replace('^', '').replace('.', '_') + '_speed_demon_v3_1')
    elif mode == 'csv':
        ...
        save_base = os.path.splitext(path5)[0] + '_speed_demon_v3_1'
    else:
        print(f"ERROR: Unknown DATA_MODE '{DATA_MODE}'"); sys.exit(1)

    print(f"\n{'─'*62}")
    print("  STRATEGY CONFIGURATION  (v3.1)")
    print(f"{'─'*62}")
    ...
    print(f"  ADX Filter (pullback)           : ADX({ADX_PERIOD}) >= {ADX_MIN}")
    ...
    print(f"  Max Trades / Day (shared)       : {MAX_TRADES_PER_DAY}")
    ...
    print(f"{'─'*62}")
    print(f"  EMA ACCELERATION BREAKOUT  [NEW v3.1]")
    print(f"{'─'*62}")
    print(f"  Master Toggle                   : {'✅ ON' if ENABLE_BREAKOUT else '❌ OFF'}  ← ENABLE_BREAKOUT")
    print(f"  ADX Min (breakout)              : {BREAKOUT_ADX_MIN}  (stricter than pullback {ADX_MIN})")
    print(f"  EMA Gap Lookback                : {BREAKOUT_GAP_LOOKBACK} bars")
    print(f"  ADX Rising Lookback             : {BREAKOUT_ADX_LOOKBACK} bars")
    print(f"  Long SL Mult                    : {BREAKOUT_LONG_SL_MULT}× ATR")
    print(f"  Short SL Mult                   : {BREAKOUT_SHORT_SL_MULT}× ATR")
    print(f"  Trade Counter                   : Shared with pullback trades")
    print(f"  Overlap Guard                   : abs(close - EMA9) > retest_tol")
    print(f"{'─'*62}")
    ...
    print(f"\nComputing 5m indicators + ADX + Breakout signals ...")
