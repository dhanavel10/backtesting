...
# ════════════════════════════════════════════════════════════════
# NEW: PRINT ALL RAW CLUSTERS
# ════════════════════════════════════════════════════════════════

def print_cluster_summary(clusters: list, current_price: float, cluster_tolerance: float):
    """
    Print every raw price cluster (before quality filters) to the console.
    Shows: cluster mean price, min/max range, spread, pivot count,
           pivot types (H/L/Both), sessions, and distance from CMP.
    """
    sep = "═" * 110
    hdr = (f"  {'#':>4}  {'Mean Price':>10}  {'Min':>8}  {'Max':>8}  "
           f"{'Spread':>7}  {'NPivots':>8}  {'Types':<10}  "
           f"{'Sessions':>8}  {'Dist from CMP':>14}  Dates (first→last)")
    div = "  " + "─" * 104

    print("\n" + sep)
    print(f"  ALL RAW CLUSTERS  ({len(clusters)} total)  "
          f"—  tolerance={cluster_tolerance} pts  |  CMP={current_price:.2f}")
    print(sep)
    print(hdr)
    print(div)

    for i, cl in enumerate(sorted(clusters, key=lambda c: c["price"]), 1):
        ...
        n_sess  = len(set(cl["sessions"]))
        dist    = cl["price"] - current_price
        sign    = "▲ RES" if cl["price"] >= current_price else "▼ SUP"

        # Earliest and latest date in cluster
        if cl["dates"]:
            sorted_dates = sorted(cl["dates"])
            d_first = str(sorted_dates[0])[:10]
            d_last  = str(sorted_dates[-1])[:10]
            date_range = f"{d_first} → {d_last}"
        else:
            date_range = "—"

        print(
            f"  {i:>4}  {cl['price']:>10.2f}  {cl['price_min']:>8.2f}  {cl['price_max']:>8.2f}  "
            f"{cl['spread']:>7.1f}  {cl['n_pivots']:>8}  {ttype:<10}  "
            f"{n_sess:>8}  {dist:>+14.1f}  {sign}  {date_range}"
        )

    print(sep)
    print(f"  Columns: Spread=pts between cluster min/max | NPivots=pivot count in cluster\n"
          f"           Types=High/Low/Both pivots | Sessions=unique trading days\n"
          f"           Dist from CMP=signed distance (+ above, − below)\n")


# ════════════════════════════════════════════════════════════════
# NEW: PLOT ALL RAW CLUSTERS ON CHART
# ════════════════════════════════════════════════════════════════

def plot_all_clusters(df: pd.DataFrame, clusters: list, zones_df: pd.DataFrame,
                      ticker: str = "", zone_half_band: float = 15.0):
    """
    Two-panel Plotly chart:
      Left panel  — all raw clusters (grey bands, labelled with cluster index)
      Right panel — only the quality-filtered precision zones (coloured bands)
    Both share the same 5m candlestick / volume layout.
    Cluster opacity is fixed (not strength-driven, since clusters are unscored).
    """
    ...
    fig = make_subplots(
        ...
        subplot_titles=("All Raw Clusters", "Precision Zones (filtered)"),
    )

    def add_candles_volume(col):
        ...
        fig.add_trace(go.Bar(
            ...
            marker_color=vcol, opacity=0.45, name="Volume", showlegend=False,
        ), row=2, col=col)

    # ── Candles on both panels ──────────────────────────────
    add_candles_volume(1)
    add_candles_volume(2)
    ...
    # ── CMP line on both panels ─────────────────────────────
    for col in (1, 2):
        fig.add_hline(y=current_price,
                      line_color="rgba(255,235,59,0.85)",
                      line_width=1.5, line_dash="dash", row=1, col=col)
        fig.add_annotation(
            x=df_plot.index[-1], y=current_price, text=f"CMP {current_price:.2f}",
            showarrow=False, xanchor="left", yanchor="bottom",
            font=dict(size=10, color="rgba(255,235,59,0.9)"),
            row=1, col=col,
        )

    # ── LEFT PANEL: all raw clusters (grey) ────────────────
    sorted_clusters = sorted(clusters, key=lambda c: c["price"])
    for idx, cl in enumerate(sorted_clusters, 1):
        level = cl["price"]
        upper = level + zone_half_band
        lower = level - zone_half_band
        ...
        fig.add_hline(y=level,
                      ...
                      line_width=1.2, line_dash="solid", row=1, col=1)

        label = (f"C{idx} {level:.0f}  [{lower:.0f}–{upper:.0f}]  "
                 f"n={cl['n_pivots']} {ttype}  spd={cl['spread']:.0f}")
        fig.add_annotation(
            x=df_plot.index[-1], y=level, text=label,
            showarrow=False, xanchor="left", yanchor="middle",
            font=dict(size=8, color="rgba(200,200,200,0.85)"),
            bgcolor="rgba(0,0,0,0.55)", borderpad=2,
            row=1, col=1,
        )

    # ── RIGHT PANEL: precision zones (coloured) ─────────────
    for _, z in zones_df.iterrows():
        ...
        fig.add_hrect(y0=z["lower"], y1=z["upper"],
                      fillcolor=fc, line_width=0, row=1, col=2)
        fig.add_hline(y=z["price"], line_color=lc,
                      line_width=1.5, line_dash="solid", row=1, col=2)
    ...
    band = int(zone_half_band * 2)
    fig.update_layout(
        title=(f"{ticker} — Raw Clusters vs Precision Zones  "
               f"(5m · ±{zone_half_band:.0f} pt bands · last 10 sessions)  "
               f"| {len(clusters)} clusters → {len(zones_df)} zones"),
        xaxis_rangeslider_visible=False,
        template="plotly_dark",
        height=860,
        showlegend=False,
        margin=dict(l=60, r=300, t=70, b=40),
        plot_bgcolor="#0d1117",
        paper_bgcolor="#0d1117",
    )
    for col in (1, 2):
        fig.update_yaxes(title_text="Nifty 50", row=1, col=col,
                         gridcolor="rgba(255,255,255,0.04)")
        fig.update_yaxes(title_text="Volume", row=2, col=col)
...
def build_precision_zones(
    ...
    # ── NEW: Print ALL raw clusters ──────────────────────────────
    current_price = float(df["Close"].iloc[-1])
    print_cluster_summary(clusters, current_price, cluster_tolerance)
    ...
    # ── NEW: return clusters too so we can plot them ─────────────
    return zones_df, clusters
...
def print_zone_summary(zones_df: pd.DataFrame, current_price: float):
    ...
    hdr = (f"  {'Type':<10} {'Price':>8} {'Lower':>8} {'Upper':>8} {'Band':>5} "
           ...
           f"{'Sess':>5} {'Holds':>6} {'Recv%':>6} {'Conv':>5} {'Dist':>7}")
    div = "  " + "─" * 94
    ...
    print(f"  Columns: WkTch=wick touches | BdyRej=body rejections | "
          ...
          f"Conv=pivot type | Dist=pts from CMP\n")


# ════════════════════════════════════════════════════════════════
# 7. INTERACTIVE CHART (original single-panel version — unchanged)
# ════════════════════════════════════════════════════════════════
...
if __name__ == "__main__":
    ...
    DAYS       = 60         # max 60 days for 5m data
    ...
    # build_precision_zones now returns (zones_df, clusters)
    zones, clusters = build_precision_zones(
        df,
        left_bars         = LEFT_BARS,
    ...
    # Original single-panel precision-zones chart (unchanged)
    fig_zones = plot_zones(df, zones, ticker=TICKER)

    # NEW: Side-by-side chart — all raw clusters (left) vs precision zones (right)
    fig_clusters = plot_all_clusters(
        df, clusters, zones,
        ticker=TICKER,
        zone_half_band=ZONE_HALF_BAND,
    )
    ...
    # fig_zones.write_html("nifty_sr_precision.html")
    # fig_clusters.write_html("nifty_sr_clusters_vs_zones.html")
